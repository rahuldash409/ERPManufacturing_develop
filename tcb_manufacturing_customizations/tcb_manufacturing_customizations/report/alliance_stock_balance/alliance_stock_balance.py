...
class StockBalanceReport:
    ...
    def prepare_new_data(self):
        ...
        # Get packed bags qty from Available Bales
        packed_bags_data = self.get_packed_bags_qty()
        ...
        for _key, report_data in self.item_warehouse_map.items():
            ...
            item_group = frappe.db.get_value(
                "Item", report_data.item_code, "item_group"
            )

            packed_qty = 0.0

            if item_group == "ad*star bags segregated":
                # Get packed qty for this item from Available Bales
                packed_qty = packed_bags_data.get(report_data.item_code, 0.0)

                # Deduct packed qty from balance qty
                report_data.bal_qty = flt(report_data.bal_qty) - flt(packed_qty)
            ...
            # Add packed bags qty to report
            report_data.update({"packed_bags_qty": packed_qty})
    ...
    def get_packed_bags_qty(self):
        """
        Get packed bags qty from Bales with Available status
        Returns dict with item_code: total_qty
        """
        packed_bags_qty = {}

        # Get all Available Bales
        bales_list = frappe.get_all(
            "Bales",
            filters={
                # "workflow_state": "Available",
                "bales_status": ["in", ("Packed In House", "Packed Import")],
                "docstatus": 1,
            },
            fields=["name", "item"],
        )
        ...
        for bale in bales_list:
            # Get batches from this Bale
            batches_used = frappe.get_all(
                "Bales Batches",
                filters={"parent": bale.name},
                fields=["item", "qty_taken"],
            )

            for batch_entry in batches_used:
                item_code = batch_entry.item
                qty = flt(batch_entry.qty_taken)

                # Check if item is in segregated group
                item_group = frappe.db.get_value("Item", item_code, "item_group")

                if item_group == "ad*star bags segregated":
                    packed_bags_qty[item_code] = (
                        packed_bags_qty.get(item_code, 0.0) + qty
                    )

        return packed_bags_qty
    ...
    def get_group_by_key(self, row) -> tuple:
        ...
        for fieldname in self.inventory_dimensions:
            ...
            if self.filters.get(fieldname) or self.filters.get(
                "show_dimension_wise_stock"
            ):
    ...
    def get_columns(self):
        ...
        columns.extend(
            [
                ...
                # },
                # {
                #     "label": _("Packed Bags Qty"),
                #     "fieldname": "packed_bags_qty",
                #     "fieldtype": "Float",
                #     "width": 150,
                # },
                # {
