...
class StockBalanceReport:
	...
	def run(self):
		...
		# print('qqqqqqq111111111111111111111111=====data  ===',self.data)
	...
	def prepare_new_data(self):		
		...
		# print('======= item_warehouse_map ==',self.item_warehouse_map)
		...
		for _key, report_data in self.item_warehouse_map.items():
			...
			# Prepare stock balance for segregated goods
			varsum = 0
			b_list = frappe.get_list("Batch",filters= {"item":report_data.item_code,"batch_qty":[">",0]},fields=["custom_segregated_item_qty"])
			for batch in b_list:
				varsum += batch.custom_segregated_item_qty
	...
	def get_item_warehouse_map(self):
		...
		for entry in self.sle_entries:
			group_by_key = self.get_group_by_key(entry)
			# print('====== entry ---', entry)
			# print('==================== group by key ==',group_by_key)
			if group_by_key not in item_warehouse_map:
				self.initialize_data(item_warehouse_map, group_by_key, entry)
				# print("=========== her ei ths  item_warehouse_map =",item_warehouse_map)
			# print('=============== item_warehouse_map  brofreee ======', item_warehouse_map)
			self.prepare_item_warehouse_map(item_warehouse_map, entry, group_by_key)
			# print('=============== item_warehouse_map  afterr------ ======', item_warehouse_map)
	...
	def initialize_data(self, item_warehouse_map, group_by_key, entry):
		...
		rack_location_str = ""
		if self.fetch_default_racks_warehouse() == entry.warehouse:
			rack_docs = frappe.get_all("Spare Rack Locations",
				filters={'parent': entry.item_code},
				fields=["combined_location"],
				pluck="combined_location"
			)
			rack_location_str = ", ".join(rack_docs) if rack_docs else ""
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				"item_code": entry.item_code,
				"warehouse": entry.warehouse,
				# "rack_location": ", ".join(entry.rack_location) if self.fetch_default_racks_warehouse() == entry.warehouse else "",
				"rack_location": rack_location_str,
	...
	def fetch_default_racks_warehouse(self):
		spares_settings_doc = frappe.get_doc('Workstation Spares Settings','Workstation Spares Settings',);
		default_racks_warehouse_name =  spares_settings_doc.default_racks_warehouse
		if (not default_racks_warehouse_name):
			frappe.throw(
			...
			return default_racks_warehouse_name
	def prepare_stock_ledger_entries(self):
		...
		query = (
			...
			.where((sle.docstatus < 2) & (sle.is_cancelled == 0) & (item_table.item_group.like("%spare%"))
										# & (sle.warehouse == self.fetch_default_racks_warehouse())
					)
	...
	def get_columns(self):
		...
		columns.extend(
			[
				...
				{
					"label": _("Segregated Goods Qty"),
					"fieldname": "segregated_goods_qty",
					"fieldtype": "Float",
					"width": 150,
				},
				{
